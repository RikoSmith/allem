    ...
    function(err, client) {
      ...
      collection.findOne(
        ...
        function(err, doc) {
          ...
          if (doc) {
            res.send(
              'There is already user registered with such username or email.'
            );
          } else {
    ...
    for (let i = 0; i < result.length; i++) {
      Member.findOne({ _id: new ObjectId(result[i].head_id) }, function(
        ...
        if (promises === result.length) {
          res.render('sb-admin/departments', {
            deps: result,
            user: req.session.user
          });
        }
...
router.get(
  ...
  function(req, res) {
    Member.findOne({ _id: new ObjectId(req.params.member_id) }, function(
      ...
      if (err) throw err;
      if (req.session.user.permission_members.indexOf(doc.dep_name) < 0)
        res.render('sb-admin/denied');
